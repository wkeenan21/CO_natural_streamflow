...
# 2. Setup Page Layout
ui.page_opts(title="Upper Colorado River Basin Streamflow, Diversions, and Consumptive Use", fillable=True)
    ...
    with ui.card():
        ...
        def map():
                    ...
                    if not selected_dvrs.empty:
                        for cu_type in selected_dvrs['siteUse'].unique():
                            ...
                            diversions_layer = L.GeoData(
                                ...
                                style={"radius": 4, "fillOpacity": 0.2, "weight": 1, "color": color},
                                interactive=False,
                                name=cu_type
                            )
            ...
            def div_click(event=None, feature=None, **kwargs):
                ...
                # Extract property name and coordinates
                ...
                # Clear existing popups from map layers
                for layer in list(m.layers):
                    if isinstance(layer, L.Popup):
                        m.remove_layer(layer)

                # Create HTML content widget for the popup
                popup_content = widgets.HTML(
                    value=f"{point_name}"
                )

                # Instantiate and display the popup
                popup = L.Popup(
                    location=[lat, lon],
                    child=popup_content,
                    close_button=True,
            ...
            # show me the active layers and controls
            for layer in list(m.layers):
    ...
    with ui.card():
        ...
        def plot_flows():
            ...
            if q_nat in df.columns:
                name = f'Observed Flow + Consumptive Use ({unit})'
                fig.add_trace(go.Scatter(
                    x=df['date'], y=df[q_nat], name=name, mode='lines',
            ...
            fig.update_layout(
                title=dict(text=f'{name} (Gage: {gage}), Period of record {valid_range[0]} to {valid_range[1]}', font=dict(size=16), subtitle=subtitle),
